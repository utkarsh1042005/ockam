...
function ockam_bump() {
  set -e
  gh workflow run release-create-pull-request.yml --ref develop -F branch_name="$release_name" -F git_tag="$GIT_TAG" -F ockam_bump_modified_release="$OCKAM_BUMP_MODIFIED_RELEASE" \
  ...
  workflow_file_name="release-create-pull-request.yml"
...
function homebrew_repo_bump() {
  ...
  gh workflow run release-create-pull-request.yml --ref main -F binaries="$file_and_sha" -F branch_name="$release_name" -F tag="$tag" -R $owner/homebrew-ockam
  # Wait for workflow run
  sleep 10
  run_id=$(gh run list --workflow=release-create-pull-request.yml -b main -u "$GITHUB_USERNAME" -L 1 -R $owner/homebrew-ockam --json databaseId | jq -r .[0].databaseId)
...
function terraform_repo_bump() {
  set -e
  gh workflow run release-create-pull-request.yml --ref main -R $owner/terraform-provider-ockam -F tag="$1" -F branch_name="$release_name"
  # Wait for workflow run
  sleep 10
  run_id=$(gh run list --workflow=release-create-pull-request.yml -b main -u "$GITHUB_USERNAME" -L 1 -R $owner/terraform-provider-ockam --json databaseId | jq -r .[0].databaseId)
